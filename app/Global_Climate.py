    ...
    if data:
        ...
        if emissions_data:
            ...
            if section == "Overview":
                ...
                # Clean emissions_df before plotting
                emissions_df = emissions_df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Emissions (tons)"])
                ...
                # Clean pie_data before plotting
                pie_data = pie_data.replace([np.inf, -np.inf], np.nan).dropna(subset=["Proportion"])
                pie_chart = px.pie(
                    pie_data,
                    names='Gas',
                    values='Proportion',
                ...
                # Clean trend_data before plotting
                trend_data = trend_data.replace([np.inf, -np.inf], np.nan).dropna(subset=["CO2", "CH4", "N2O"])
                ...
                # --- SARIMAX Example for CO2 Time Series ---
                st.write("### Forecasting Example: SARIMAX on CO2 Emissions")

                # Prepare the CO2 series
                co2_series = trend_data['CO2'].replace([np.inf, -np.inf], np.nan).dropna()

                # Check for stationarity and difference if needed
                adf_result = adfuller(co2_series)
                if adf_result[1] > 0.05:
                    st.info("CO2 series is non-stationary, differencing will be applied.")
                    co2_series = co2_series.diff().dropna()

                # Fit SARIMAX model with simple order, increase maxiter
                try:
                    model = sm.tsa.SARIMAX(co2_series, order=(1,1,1), enforce_stationarity=False, enforce_invertibility=False)
                    results = model.fit(maxiter=500, disp=False)
                    st.success("SARIMAX model fit successfully.")
                    # Forecast next 5 periods
                    forecast = results.forecast(steps=5)
                    st.write("Next 5-period forecast:", forecast.values)
                except Exception as e:
                    st.warning(f"SARIMAX model failed: {e}")
                ...
                fig = go.Figure(go.Indicator(
                    mode="gauge+number",
                    value=co2 / (co2 + ch4 + n2o) * 100 if (co2 + ch4 + n2o) != 0 else 0,
                ...
                # Clean comparison_df before plotting
                comparison_df = comparison_df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Emissions (tons)"])
                bar_chart = px.bar(
                    comparison_df,
                    x='Country',
                    y='Emissions (tons)',
                ...
                # Clean map_df before plotting
                map_df = map_df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Emissions (tons)", "Latitude", "Longitude"])
...
# df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Emissions_start", "Emissions_end"])
